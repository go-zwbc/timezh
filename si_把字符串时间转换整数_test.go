...
func Test_si把字符串时间转换整数_Get通过时间得到整数(t *testing.T) {
	...
	fmt.Println(v年份, v月份, v几号, V几时, v几分, v秒数)
}

func Test_si把字符串时间转换整数_Get通过时间得到整数1(t *testing.T) {
	Y, month, D, H, minute, S := SI.Get通过时间得到整数("2018-01-02 03:04:05")
...
func Test_si把字符串时间转换整数_Get通过时刻得到几时几分秒数1(t *testing.T) {
	H, M, S := SI.Get通过时刻得到几时几分秒数("09:08:07")
	if H != 9 || M != 8 || S != 7 {
		t.Error(H, M, S)
		return
	}
